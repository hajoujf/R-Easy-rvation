...
if (isset($_GET['id']) && !empty($_GET['id'])) {
    ...
    $sql = "SELECT * FROM tborder WHERE OrderID = '$orderId' AND Username = '$_SESSION[User]'";
    ...
    if (mysqli_num_rows($result) == 1) {
        ...
        $deleteSql = "DELETE FROM tborder WHERE OrderID = '$orderId' AND Username = '$_SESSION[User]'";
        ...
        if ($deleteResult && $deleteResult2) {
            ...
            if (($ownerEmailResult && mysqli_num_rows($ownerEmailResult) == 1) && ($result_tbuser && mysqli_num_rows($result_tbuser) == 1)) {
                ...
                mail($restaurantOwnerEmail, $subject, $message, $header);
            }
            ...
            echo "<script>window.location.href = '../OrdersClientHistory.php';</script>";
            exit();
        } else {
